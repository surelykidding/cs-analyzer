...
- 经济局筛选：`Pistol`、`Eco`、`Semi`、`ForceBuy`、`Full`
...
如果这个命令失败，请先安装 PostgreSQL 客户端工具，再重新打开终端。
...
你也可以使用自己的数据库参数，但为了减少排查成本，建议先按默认值测试。

## 第一次启动

1. 启动 `CS Analyzer`。
2. 如果出现数据库连接界面，填入你的 PostgreSQL 参数。
3. 对于全新环境，应用会自动创建数据库并执行 migration。
...
- 使用你 FACEIT 个人主页里的准确昵称。
- 如果添加了多个账号，确认当前账号切换正确。

## FACEIT Scouting 测试流程
...
- 你测试的是 `FACEIT`、`Team Tactics` 还是 `Perfect World`
