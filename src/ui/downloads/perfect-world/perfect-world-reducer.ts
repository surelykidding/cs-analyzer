...
import {
  ...
  matchSelected,
} from './perfect-world-actions';
import type { PerfectWorldMatch } from 'csdm/common/types/perfect-world-match';

type State = {
  readonly status: Status;
  readonly accounts: PerfectWorldAccount[];
  readonly matches: PerfectWorldMatch[];
...
export const perfectWorldReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(downloadsAdded, (state, action) => {
      for (const download of action.payload.downloads) {
        if (download.source !== DownloadSource.PerfectWorld) {
          continue;
        }
    ...
    .addCase(initializeAppSuccess, (state, action) => {
      state.accounts = action.payload.perfectWorldAccounts;
    })
    .addCase(accountAdded, (state, action) => {
      state.accounts = state.accounts
        .filter((account) => account.id !== action.payload.account.id)
        .map((account) => {
          return {
            ...account,
            isCurrent: false,
