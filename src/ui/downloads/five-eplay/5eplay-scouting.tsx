...
import {
  ...
  type FiveEPlayScoutingTacticsResponse,
...
function TargetStatusBadge({ status }: { status: FiveEPlayScoutingSession['targets'][number]['status'] }) {
  const className =
...
export function FiveEPlayScouting() {
  ...
  const [session, setSession] = useState<FiveEPlayScoutingSession | undefined>(undefined);
  ...
  const [response, setResponse] = useState<FiveEPlayScoutingTacticsResponse | undefined>(undefined);
  ...
  const loadSession = async () => {
    try {
      ...
      const currentSession = await client.send({
        name: RendererClientMessageName.FetchCurrent5EPlayScoutingSession,
      });
  ...
  useEffect(() => {
    const onSessionUpdated = (nextSession: FiveEPlayScoutingSession | undefined) => {
      setSession(nextSession);
      setSessionStatus(Status.Success);
    };

    client.on(RendererServerMessageName.FiveEPlayScoutingSessionUpdated, onSessionUpdated);

    return () => {
      client.off(RendererServerMessageName.FiveEPlayScoutingSessionUpdated, onSessionUpdated);
    };
  }, [client]);
    ...
    const fetchTactics = async () => {
      try {
        ...
        const tactics = await client.send({
          name: RendererClientMessageName.Fetch5EPlayScoutingTactics,
          payload: {
        ...
        showToast({
          id: '5eplay-scouting-tactics-error',
          type: 'error',
  ...
  const startSession = async () => {
    try {
      ...
      const nextSession = await client.send({
        name: RendererClientMessageName.Start5EPlayScoutingSession,
        payload: {
          matchIdOrUrl,
        },
      });
      ...
      setMatchIdOrUrl('');
      showToast({
        id: '5eplay-scouting-session-started',
        type: 'success',
        content: <Trans>5EPlay scouting session started.</Trans>,
      });
    } catch (error) {
  ...
  const deleteSession = async () => {
    ...
    try {
      ...
      await client.send({
        name: RendererClientMessageName.Delete5EPlayScoutingSession,
        payload: {
          sessionId: session.id,
        },
      });
      ...
      setResponse(undefined);
      showToast({
        id: '5eplay-scouting-session-deleted',
        type: 'success',
        content: <Trans>5EPlay scouting session deleted.</Trans>,
      });
    } catch (error) {
      showToast({
  ...
  const generatePositionsForSkippedMatches = async () => {
    ...
    try {
      ...
      showToast({
        id: '5eplay-scouting-positions-generated',
        type: 'success',
        content: <Trans>Tactics positions generated for imported scouting demos.</Trans>,
      });
    } catch {
      showToast({
        id: '5eplay-scouting-positions-generated-error',
        type: 'error',
  ...
  if (sessionStatus === Status.Loading && session === undefined) {
    return <Message message={<Trans>Loading 5EPlay scouting...</Trans>} />;
  }
        ...
        <HeatmapFilters>
          <div className="rounded-8 border border-gray-300 bg-gray-50 p-12">
            ...
            <p className="mt-8 text-caption text-gray-800">
              <Trans>
                Paste a 5EPlay room URL or match ID to find the opponent&apos;s recent matches on the same map. Matching
            ...
            <p className="mt-8 text-caption text-gray-800">
              <Trans>Current 5EPlay account: {currentAccount.nickname}</Trans>
            </p>
          </div>
          ...
          {session === undefined ? (
            <div className="rounded-8 border border-gray-300 bg-gray-50 p-12">
              <TextInput
                label={<Trans context="Input label">5EPlay room URL or match ID</Trans>}
                value={matchIdOrUrl}
                onChange={(event) => {
                  setMatchIdOrUrl(event.target.value);
                }}
                placeholder={t({
                  message: 'https://arena.5eplay.com/data/match/...',
              ...
              <div className="rounded-8 border border-gray-300 bg-gray-50 p-12">
                ...
                <div className="mt-12 flex flex-wrap gap-8">
                  <OpenLinkButton url={session.sourceMatch.url}>
                    <Trans>See source room on 5EPlay</Trans>
                  </OpenLinkButton>
                  <Button onClick={loadSession}>
