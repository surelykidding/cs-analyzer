...
import { fetch5EPlayScoutingSession } from './fetch-5eplay-scouting-session';

function emptyResponse(payload: FiveEPlayScoutingTacticsPayload): FiveEPlayScoutingTacticsResponse {
  return {
    side: payload.side,
...
export async function fetch5EPlayScoutingTactics(
  payload: FiveEPlayScoutingTacticsPayload,
): Promise<FiveEPlayScoutingTacticsResponse> {
  const session = await fetch5EPlayScoutingSession(payload.sessionId);
