import type { FaceitScoutingTacticsPayload, FaceitScoutingTacticsResponse } from 'csdm/common/types/faceit-scouting';
import { fetchFaceitScoutingSession } from './fetch-faceit-scouting-session';
import { fetchTeamTactics } from '../team-tactics/fetch-team-tactics';

function emptyResponse(payload: FaceitScoutingTacticsPayload): FaceitScoutingTacticsResponse {
  return {
    side: payload.side,
...
export async function fetchFaceitScoutingTactics(
  payload: FaceitScoutingTacticsPayload,
): Promise<FaceitScoutingTacticsResponse> {
  const session = await fetchFaceitScoutingSession(payload.sessionId);
