...
const (
	...
	positionEntityPlayers  = "players"
	positionEntityGrenades = "grenades"
	positionEntityInfernos = "infernos"
	positionEntityHostages = "hostages"
	positionEntityChickens = "chickens"
	positionEntityAll      = "all"
)
...
type Analyzer struct {
	...
	selectedRounds          map[int]struct{}
	positionEntities        map[string]struct{}
	positionWindowStartSecs int
	positionWindowEndSecs   int
}
...
func validPositionEntities() []string {
	return []string{
		positionEntityAll,
		positionEntityPlayers,
		positionEntityGrenades,
		positionEntityInfernos,
		positionEntityHostages,
		positionEntityChickens,
	}
}

func FormatValidPositionEntities() string {
	return stdstrings.Join(validPositionEntities(), ", ")
}

func ValidatePositionEntities(entities []string) error {
	if len(entities) == 0 {
		return nil
	}

	validEntities := make(map[string]struct{}, len(validPositionEntities()))
	for _, entity := range validPositionEntities() {
		validEntities[entity] = struct{}{}
	}

	for _, entity := range entities {
		normalizedEntity := stdstrings.TrimSpace(stdstrings.ToLower(entity))
		if _, isValid := validEntities[normalizedEntity]; !isValid {
			return fmt.Errorf("invalid position entity %q, valid values are: %s", entity, FormatValidPositionEntities())
		}
	}
...
func validateRoundNumbers(roundNumbers []int) error {
	for _, roundNumber := range roundNumbers {
		if roundNumber <= 0 {
			return fmt.Errorf("round number must be greater than 0, got %d", roundNumber)
		}
	}
...
func ValidatePositionWindowSeconds(startSeconds int, endSeconds int) error {
	if startSeconds < 0 {
		return fmt.Errorf("position window start seconds must be greater than or equal to 0, got %d", startSeconds)
	}

	if endSeconds < 0 {
		return fmt.Errorf("position window end seconds must be greater than or equal to 0, got %d", endSeconds)
	}

	if endSeconds > 0 && startSeconds >= endSeconds {
		return fmt.Errorf("position window start seconds must be lower than end seconds, got %d and %d", startSeconds, endSeconds)
	}

	return nil
}

func newSelectedRounds(roundNumbers []int) map[int]struct{} {
	if len(roundNumbers) == 0 {
		return nil
	}

	selectedRounds := make(map[int]struct{}, len(roundNumbers))
	for _, roundNumber := range roundNumbers {
		selectedRounds[roundNumber] = struct{}{}
	}

	return selectedRounds
}

func newPositionEntities(entities []string) map[string]struct{} {
	if len(entities) == 0 {
		return nil
	}

	positionEntities := make(map[string]struct{}, len(entities))
	for _, entity := range entities {
		normalizedEntity := stdstrings.TrimSpace(stdstrings.ToLower(entity))
		positionEntities[normalizedEntity] = struct{}{}
	}

	if _, hasAll := positionEntities[positionEntityAll]; hasAll {
		return nil
	}
...
func analyzeDemo(demoPath string, options AnalyzeDemoOptions) (*Match, error) {
	...
	if err := validateRoundNumbers(options.RoundNumbers); err != nil {
		return nil, err
	}

	if err := ValidatePositionEntities(options.PositionEntities); err != nil {
		return nil, err
	}

	if err := ValidatePositionWindowSeconds(options.PositionWindowStartSeconds, options.PositionWindowEndSeconds); err != nil {
		return nil, err
	}
	...
	analyzer := &Analyzer{
		...
		selectedRounds:            newSelectedRounds(options.RoundNumbers),
		positionEntities:          newPositionEntities(options.PositionEntities),
		positionWindowStartSecs:   options.PositionWindowStartSeconds,
		positionWindowEndSecs:     options.PositionWindowEndSeconds,
	}
	...
	if len(options.RoundNumbers) > 0 {
		match.filterRounds(options.RoundNumbers)
	}
...
func AnalyzeAndExportDemo(demoPath string, outputPath string, options AnalyzeAndExportDemoOptions) error {
	...
	match, err := analyzeDemo(demoPath, AnalyzeDemoOptions{
		...
		Source:                     options.Source,
		RoundNumbers:               options.RoundNumbers,
		PositionEntities:           options.PositionEntities,
		PositionWindowStartSeconds: options.PositionWindowStartSeconds,
		PositionWindowEndSeconds:   options.PositionWindowEndSeconds,
	})
...
func (analyzer *Analyzer) shouldIncludeCurrentRound() bool {
	if len(analyzer.selectedRounds) == 0 {
		return true
	}

	_, shouldInclude := analyzer.selectedRounds[analyzer.currentRound.Number]
	return shouldInclude
}

func (analyzer *Analyzer) shouldIncludePositionEntity(entity string) bool {
	if len(analyzer.positionEntities) == 0 {
		return true
	}

	_, shouldInclude := analyzer.positionEntities[entity]
	return shouldInclude
}

func (analyzer *Analyzer) shouldCapturePositionsAtCurrentTick() bool {
	if analyzer.currentRound.FreezeTimeEndTick == -1 {
		return false
	}

	secondsSinceFreezeTimeEnd := float64(analyzer.currentTick()-analyzer.currentRound.FreezeTimeEndTick) * analyzer.parser.TickTime().Seconds()
	if analyzer.positionWindowStartSecs > 0 && secondsSinceFreezeTimeEnd < float64(analyzer.positionWindowStartSecs) {
		return false
	}

	if analyzer.positionWindowEndSecs > 0 && secondsSinceFreezeTimeEnd >= float64(analyzer.positionWindowEndSecs) {
		return false
	}
...
func (analyzer *Analyzer) registerCommonHandlers(includePositions bool) {
	...
	if includePositions {
		parser.RegisterEventHandler(func(event events.FrameDone) {
			if !analyzer.matchStarted() || !analyzer.shouldIncludeCurrentRound() || !analyzer.shouldCapturePositionsAtCurrentTick() {
				return
			}

			if analyzer.shouldIncludePositionEntity(positionEntityChickens) {
			...
			if analyzer.shouldIncludePositionEntity(positionEntityGrenades) {
			...
			if analyzer.shouldIncludePositionEntity(positionEntityInfernos) {
			...
			if analyzer.shouldIncludePositionEntity(positionEntityPlayers) {
			...
			if analyzer.shouldIncludePositionEntity(positionEntityHostages) {
